...
### 🗓️ 2026-06-17 (v1.0.1 真正并发 + 质量提升)

#### 🔄 架构变更

| 项目 | 变更 |
|------|------|
| **并发模型** | 移除全局锁 `_search_lock`，改用 thread-local 独立 ChromiumPage + 信号量控制并发上限 |
| **端口分配** | `_get_port()` PID+随机数 → `_alloc_port()` 线程安全计数器，保证端口唯一不冲突 |
| **Edge 路径** | 每次调用都搜索 → `@lru_cache` 缓存，只查找一次 |
| **返回格式** | 纯文本 → JSON 结构化数据（含 title/url/snippet/date/source/rank） |
| **搜索方式** | 直接 URL 搜索 → 先访问首页输入关键词（模拟真人），失败回退 URL 方式 |
| **空白页容错** | 无 → 检测 `#b_results` 缺失时自动刷新重试最多 3 次 |
| **滚动加载** | 无 → 滚动 3 次加载更多结果 |
| **web_fetch 提取** | 纯 DrissionPage DOM 提取 → trafilatura 智能正文提取 + 失败回退 |

#### 🚀 性能优化

| 优化项 | 之前 | 之后 |
|--------|------|------|
| **并发上限** | 无限制（全局锁串行） | 最多 4 个浏览器同时运行 |
| **超时参数** | `ele_displayed` 10s / 子元素 1s | `ele_displayed` 3s / 子元素 0.5s / 单元素 0.3s |
| **日期提取** | 单独 DOM 查询 | 优先从 snippet 正则提取，零 DOM 开销 |
| **web_fetch 重试** | 无 | 失败自动重试最多 3 次 |

#### 🐛 修复记录

| 问题 | 症状 | 根因 | 解决方案 |
|------|------|------|----------|
| **全局锁串行** | 并发搜索排队等待，无法真正并行 | `_search_lock` 锁住整个搜索过程 | 移除全局锁，每个线程独立管理 ChromiumPage |
| **端口冲突** | 并发时 `Handshake status 404` | PID+随机数仍有小概率碰撞 | `_alloc_port()` 原子计数器，保证 100% 唯一 |
| **搜索结果不完整** | 只返回前几条，缺少懒加载内容 | 未触发 Bing 的滚动懒加载 | 滚动 3 次 + 等待 `.b_algo` 出现 |
| **web_fetch 正文质量差** | 提取到导航/广告等非正文内容 | 纯 DOM 提取无法区分正文和噪音 | trafilatura 智能提取 + 回退机制 |
| **空结果无反馈** | 搜索返回空字符串 | 无空白页检测逻辑 | `#b_results` 检测 + 自动刷新重试最多 3 次 |

#### ⚠️ 已知问题（后续修复）

| 问题 | 说明 |
|------|------|
| **死代码残留** | `_get_thread_page()` / `close_all_browsers()` 未被调用，属于死代码 |
| **信号量泄露** | `_get_thread_page()` acquire 后不 release，`close_all_browsers()` 只 release 一次 |
| **硬编码 sleep** | 滚动加载仍用 `time.sleep(0.5)`，与 Pitfall Notes 声称的"移除了所有冗余 time.sleep"矛盾 |
| **异常被吞** | 两处 `except Exception` 均未记录 traceback，调试困难 |
